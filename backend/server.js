...
app.post('/api/rclone-authorize', (req, res) => {
  ...
  global.rcloneAuthState = { status: 'running', output: '$ rclone authorize "google photos" --scopes "https://www.googleapis.com/auth/photoslibrary.readonly" --auth-no-open-browser\n', remoteName, readOnly: !!readOnly };

  const child = spawn('rclone', ['authorize', 'google photos', '--scopes', 'https://www.googleapis.com/auth/photoslibrary.readonly', '--auth-no-open-browser', '--config=/config/rclone.conf'], { shell: false });
